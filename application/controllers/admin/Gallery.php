...
class Gallery extends CI_Controller {
    ...
    $data['list_gallery'] = $this->Webmodel->get_gallery('',$id)->result();
    $data['id_category'] = $id;
  ...
  public function add($id_category){  
    $data['profil'] = $this->Admin_model->get_profil()->row();
    $data['lokasi'] = $this->Admin_model->get_lokasi($id_category)->result();
    $data['category_wisata'] = $this->Admin_model->get_category_wisata()->result();
    ...
    if($this->input->post()){
        ...
        if ( ! $this->upload->do_upload('image_1')){
          ...
          redirect('admin/gallery/index/'.$id_category, 'refresh');
...
public function delete($id){
  ...
  redirect('admin/gallery/index/'.$images->id_category,'refresh');   
  }
}
?>